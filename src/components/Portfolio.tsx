...
const Portfolio = () => {
  ...
  const displayedItems = useMemo(() => (showAll ? filteredItems : filteredItems.slice(0, INITIAL_DISPLAY)), [showAll, filteredItems])
  ...
  const renderErrorState = () => (
    <div className="col-span-full text-center py-16">
      ...
      <p className="text-muted-foreground mb-4">{(error as any)?.message || 'An error occurred while loading portfolio items.'}</p>
      <Button onClick={() => window.location.reload()} variant="outline" className="border-cyan/30 hover:border-cyan hover:bg-cyan/10">
        Try Again
      </Button>
    </div>
  )
  ...
  return (
    <section id="portfolio" data-testid="portfolio-section" className="py-20 bg-background">
      <div className="container mx-auto px-4 lg:px-8">
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-heading font-bold text-foreground mb-4">
            Our <span className="bg-gradient-cmyk bg-clip-text text-transparent ml-3">Portfolio</span>
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Explore our collection of premium print work showcasing quality, creativity, and attention to detail across our services.</p>
        </motion.div>

        {categories.length > 0 && (
          <motion.div className="flex flex-wrap justify-center gap-4 mb-12" role="tablist" aria-label="Filter portfolio" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            {categories.map((category) => (
              <motion.button
                ...
                className={`px-6 py-2 rounded-full text-sm font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-cyan focus:ring-offset-2 ${
                  activeFilter === category ? 'bg-primary text-primary-foreground shadow-lg' : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'
                }`}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{renderErrorState()}</div>
        ) : displayedItems.length > 0 ? (
          <motion.div id="portfolio-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" variants={containerVariants} initial="hidden" animate="visible">
            <AnimatePresence>
              {displayedItems.map((p, index) => (
                ...
                >
                  <div className="aspect-square overflow-hidden bg-gradient-to-br from-slate-700 to-slate-900">
                    {p.image ? (
                      <OptimizedImage
                        src={urlFor(p.image).width(1200).height(1200).fit('crop').format('webp').quality(85).url()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{renderEmptyState()}</div>
        )}

        {hasMore && (
          <motion.div className="text-center mt-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} viewport={{ once: true }}>
            <Button
              onClick={() => {
                setShowAll(!showAll)
        ...
        <Lightbox open={lightboxOpen} onOpenChange={setLightboxOpen} items={filteredItems} startIndex={lightboxStart} />
      </div>
    </section>
  )
}
